...
class ChildApiTest {
    ...
    void 家长可读自己监护的孩子() throws Exception {
        // 注册家长,查出其 userId
        String parentToken = AuthTestSupport.registerAndLogin(
            mvc, json, "p_self_parent", "pw123456", "PARENT", 1L);
        Long parentId = jdbc.queryForObject(
            "SELECT id FROM app_user WHERE username = ?", Long.class, "p_self_parent");

        // 老师建档并把该家长设为监护人
        String teacherToken = AuthTestSupport.registerAndLogin(
            mvc, json, "p_self_teacher", "pw123456", "TEACHER", 1L);
    ...
    void 家长不能读他人的孩子403() throws Exception {
        // 老师建一个未绑定该家长的孩子
        String teacherToken = AuthTestSupport.registerAndLogin(
            mvc, json, "p_other_teacher", "pw123456", "TEACHER", 1L);
        long childId = createChild(teacherToken, "小红", null);

        // 另一个家长(非监护人)读 → 403
        String parentToken = AuthTestSupport.registerAndLogin(
            mvc, json, "p_other_parent", "pw123456", "PARENT", 1L);
        mvc.perform(get("/api/children/" + childId).header("Authorization", "Bearer " + parentToken))
