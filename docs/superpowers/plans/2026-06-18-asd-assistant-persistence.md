...
- [ ] **Step 1: 先加契约测试(四参锁定 schools 校验)+ 三参 happy-path 测试**
...
- [ ] **Step 3: 修改三参委托,把 schools 并入 mustNotContain**
...
- [ ] **Step 3: 创建 schema.sql 雏形(空占位,后续任务追加表)**
...
- [ ] **Step 7: 提供空的 knowledgeDocs bean(脚手架,解除上下文阻塞)**
...
- [ ] **Step 1: schema.sql 追加 child 表**

在 `schema.sql` 末尾的 `-- (Task 5 追加 child 表)` 注释下追加:
```sql
...
- [ ] **Step 3: 创建 Mapper 接口(以密文列读写,DTO 用 Map 避免领域实体泄露密文语义)**
...
`backend/src/main/resources/mybatis/ChildMapper.xml`:
```xml
...
<mapper namespace="com.sellm.child.ChildMapper">
    ...
    <select id="findById" parameterType="long" resultType="map">
        SELECT id, name_enc AS nameEnc, disorder_type AS disorderType, org_id AS orgId
...
- [ ] **Step 1: schema.sql 追加 knowledge_doc 表**
...
- [ ] **Step 4: 写失败测试(H2 集成,用 JdbcTemplate 插种子数据)**
...
- [ ] **Step 2: 创建 ScaleMapper(分三查,Repository 组装)**
...
`backend/src/main/resources/mybatis/ScaleMapper.xml`:
```xml
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE mapper PUBLIC "-//mybatis.org//DTD Mapper 3.0//EN"
        "http://mybatis.org/dtd/mybatis-3-mapper.dtd">
<mapper namespace="com.sellm.scale.ScaleMapper">
    ...
    <select id="findScaleById" parameterType="string" resultType="map">
        SELECT scale_id AS scaleId, name, version FROM scale WHERE scale_id = #{scaleId}
    </select>

    <select id="findItems" parameterType="string" resultType="map">
        SELECT item_id AS itemId, stem, dimension FROM scale_item WHERE scale_id = #{scaleId}
    </select>

    <select id="findBands" parameterType="string" resultType="map">
        SELECT lower_bound AS lowerBound, upper_bound AS upperBound, label, interpretation
...
`backend/src/test/resources/scale-seed.sql`:
```sql
...
---
### Task 8: 上下文启动守护测试 + 全量回归

补一个显式的 `@SpringBootTest` 上下文测试,守住"应用能正常启动 + 关键 bean 装配正确"——这是计划一漏掉、导致启动隐患没被发现的根因。再跑全量测试确认整个持久化层与既有功能无回归。
...
class ApplicationContextTest {
    ...
    void RagRetriever默认实现为DbRagRetriever() {
        // @Primary 生效:业务拿到的是 DB 实现,而非 InMemory
...
Expected: 2 个测试 PASS。上下文启动正常,RagRetriever 注入的是 DbRagRetriever(@Primary)。

- [ ] **Step 3: 运行全量测试,确认无回归**

Run: `cd "D:/works/test/SELLM/backend" && ./mvnw -q test`
Expected: 全部 PASS。预期总数 = 计划一 22 + Task1 新增 3 + Task2 新增 2 + Task3 冒烟 1 + Task4 加密 4 + Task5 Child 3 + Task6 Rag 3 + Task7 Scale 3 + Task8 上下文 2 = 43。(若个别数字微调,以实际为准,但不得有 Failure/Error。)

- [ ] **Step 4: Commit**

```bash
cd "D:/works/test/SELLM" && git add backend/src/test/java/com/sellm/context/ && git commit -q -m "test(context): @SpringBootTest 守护应用启动与关键 bean 装配"
```
