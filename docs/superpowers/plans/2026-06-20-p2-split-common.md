...
- [x] **Step 1: 创建 sellm-common-core/pom.xml**
...
- [x] **Step 2: 根 pom 注册 core 模块 + dependencyManagement**
...
- [x] **Step 3: git mv core 类(common 基础 + crypto 接口 + event)**
...
- [x] **Step 4: git mv core 类(security 契约 + JwtService + AccessGuard)**
...
- [x] **Step 5: git mv core 类(各能力接口 + DTO)**
...
- [x] **Step 1: 创建 sellm-common-backend/pom.xml**
...
- [x] **Step 2: git mv backend 实现/配置类**
...
- [x] **Step 4: 根 pom 注册 backend 模块 + 删除旧 sellm-common 模块声明**
...
- [x] **Step 5: 删除已空的旧 sellm-common 目录**
...
- [x] **Step 6: 验证 core + backend 两模块编译通过**
...
- [x] **Step 2: 4 个 agent pom 换依赖(sellm-common → sellm-common-core)**
...
- [x] **Step 3: 去掉 4 个 agent 主 yml 的 DataSource 排除 workaround**
...
- [x] **Step 4: 去掉 4 个 agent 测试 yml 的 autoconfigure 排除**
...
- [x] **Step 5: 验证 backend 237 全绿(clean)**
...
- [x] **Step 6: 验证 4 个 agent contextLoads(无 workaround,无 Nacos)**
...
- [x] **Step 2: 确认 agent 不再传递 jdbc/mybatis/minio**
