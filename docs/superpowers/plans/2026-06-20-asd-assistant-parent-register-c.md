...
- [x] **Step 1:** `Relationship` 枚举:MOTHER_SON("母子")、MOTHER_DAUGHTER("母女")、FATHER_SON("父子")、FATHER_DAUGHTER("父女");getLabel() + valueOf 校验工具(参照 DisorderType 范式)
- [x] **Step 2:** schema.sql 追加:
```sql
...
```
- [x] **Step 3:** RelationshipTest(枚举齐全 + 标签 + 非法码抛异常)
...
- [x] **Step 1:** `GET /api/orgs/public/{orgId}/classes` → 该机构班级列表(id+name);复用 ClazzRepository.listByOrg
- [x] **Step 2:** `GET /api/classes/public/{classId}/teachers` → 该班级关联老师(userId + username);TeacherClassMapper 加 `findTeacherIdsByClass(classId)`,再查 app_user 取 username(只返回 id+username,不含敏感字段)
- [x] **Step 3:** SecurityConfig:这两个 public 端点 permitAll(放在 /api/classes、/api/orgs 既有规则之前)
- [x] **Step 4:** 测试:建机构+班级+老师绑班级 → 公开查班级含它、查班级老师含该老师(免 token)
...
- [x] **Step 1:** ParentProfile 实体 + Mapper(insert/findByUserId/updateChildId/findByOrg 关联查)+ XML;ParentProfileRepository(save 加密 name/childName,findByUserId 解密)
- [x] **Step 2:** RegisterRequest 扩:name、relationship、childName、childDisorderType、assignedTeacherId、classId(orgId 已有)
- [x] **Step 3:** AuthController.register 改造(@Transactional):
...
- [x] **Step 4:** ParentProfileRepositoryTest + ParentRegisterApiTest(注册成功落两表 / 缺老师 400 / 非法关系码 400 / 指派非本机构老师 400)
- [x] **Step 5:** 全量回归绿 → Commit
...
- [x] **Step 1:** SecurityConfig:`PUT /api/users/*/approve|reject` 从 hasRole("MANAGER") 改 hasRole("TEACHER");`GET /api/users/pending` 同步改 TEACHER(老师看分派给自己的待审)
- [x] **Step 2:** pending 列表改造:老师只看 `assigned_teacher_id == me` 的 PENDING 家长(ParentProfileRepository.findPendingByTeacher 关联 app_user.status=PENDING)
- [x] **Step 3:** approve 改造(@Transactional):
...
- [x] **Step 4:** reject 改造:行级同上(assigned_teacher_id==me);置 REJECTED;不建儿童
- [x] **Step 5:** ParentApprovalApiTest:
...
- [x] **Step 1:** ParentProfileRepository 加 `listByOrg(orgId)`:join app_user(role=PARENT, org)+ parent_profile + class_room(班级名)+ child;解密姓名/儿童姓名
- [x] **Step 2:** ParentResponse 扩:id/username/status + name/relationship(标签)/childName/className
- [x] **Step 3:** `GET /api/users/parents`(MANAGER)改为返回完整字段(无 profile 的旧家长字段留空,兼容)
- [x] **Step 4:** ParentListApiTest 升级:注册一个家长(带 profile)→ 管理员看到完整字段;他机构家长不可见
- [x] **Step 5:** 全量回归绿 → Commit
...
- [x] **Step 1:** meta.js 加 `RELATIONSHIPS`(code+label);api 加公开端点 listPublicClasses(orgId)、listClassTeachers(classId)
- [x] **Step 2:** RegisterView 扩表单:姓名、账号、密码、儿童姓名、残障类型(单选 DISORDER_TYPES)、所在学校(机构,已有)、**联动**:选机构→拉班级→选班级→拉老师→选老师;亲戚关系(RELATIONSHIPS)。提交全字段
- [x] **Step 3:** UsersView 老师区(auth.isTeacher):新增"待审核家长"卡(GET /users/pending,展示姓名/儿童/关系)+ 通过/拒绝按钮(老师只看分派给自己的)
- [x] **Step 4:** UsersView 管理员区"本机构家长"表升级列:姓名、儿童姓名、关系、班级、状态
- [x] **Step 5:** (管理员区原"待审核家长"卡移除——审核已归老师)
...
- [x] **Step 1:** 起 dev 后端(schema 新增 parent_profile,CREATE IF NOT EXISTS 兼容;必要时删 data/ 重建)
- [x] **Step 2:** curl 链路(8~10 条):公开班级/老师查询、注册落两表、老师行级待审、通过建儿童、家长登录、越权 403
