...
- [x] **Step 0: schema.sql 追加 organization 表**
...
- [x] **Step 0b: Organization 实体 + Mapper + XML + Repository**
...
- [x] **Step 0c: OrganizationRepository 测试(H2 集成)**
...
- [x] **Step 1: schema.sql 追加 app_user 表**
...
- [x] **Step 6: 运行测试确认失败** — `cd "D:/works/test/SELLM/backend" && ./mvnw -q test -Dtest=UserRepositoryTest`，编译失败(UserRepository 未建)。

- [x] **Step 7: 实现 UserRepository**
...
- [x] **Step 2: 运行确认失败** — `./mvnw -q test -Dtest=JwtServiceTest`，编译失败。

- [x] **Step 3: 实现 JwtService**
...
- [x] **Step 4: 跑测试确认通过** — 5 PASS。注:过期测试用 -1 分钟使 token 立即过期;orgId 为 null 时 claim 存 null,解析返回 null。
...
- [x] **Step 2: CurrentUser(从 SecurityContext 取主体)**
...
- [x] **Step 3: JwtAuthFilter(把 AuthPrincipal 作为 principal 放进 Authentication)**
...
- [x] **Step 4: 跑测试** — `./mvnw -q test -Dtest=AuthApiTest`，2 PASS。注:`/api/auth/**` 在 SecurityConfig 里 permitAll,登录失败经全局 advice 返回 400。
...
- [x] **Step 0: schema.sql 给 child 表加列;Child 实体加字段**
...
- [x] **Step 2: ChildMapper 加 findAll/update/delete + guardian_user_id 读写**
...
- [x] **Step 3: ChildRepository 带 guardianUserId 读写 + findAll/update/delete**
...
- [x] **Step 5: ChildController(建档绑机构/家长 + 读写行级过滤)**
...
- [x] **Step 8: 跑测试** — `./mvnw -q test -Dtest=ChildApiTest`，3 PASS。
...
- [x] **Step 1: schema.sql 追加 assessment 表**
...
- [x] **Step 6: AssessmentAppService(编排:读 Child/Scale→计分→存)**
...
- [x] **Step 9: 跑测试** — `./mvnw -q test -Dtest=AssessmentApiTest`，2 PASS。家长被端点级 RBAC 拒(403,POST 不允许 PARENT)。
...
- [x] **Step 1: schema.sql 追加 report 表**
...
- [x] **Step 4: ReportRecordRepository**
...
- [x] **Step 6: ReportAppService(读 assessment+child→调 ReportService→存记录)**
...
- [x] **Step 9: 跑测试** — `./mvnw -q test -Dtest=ReportApiTest`，1 PASS(用默认 MockAiModel,draft 非空)。
...
- [x] **Step 2: IepRecord 实体**(字段同 ReportRecord,把 assessmentId 换成 reportId)
...
- [x] **Step 4: IepRecordRepository**(镜像 ReportRecordRepository)
...
- [x] **Step 6: IepAppService(读 report+child→调 IepService→存)**
...
- [x] **Step 9: 跑测试** — `./mvnw -q test -Dtest=IepApiTest`，1 PASS。
...
- [x] **Step 3: 跑这两个测试** — `./mvnw -q test -Dtest="FullChainApiTest,AuthorizationTest"`，全 PASS。
